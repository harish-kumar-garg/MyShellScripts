...
cat "sonar.jdbc.username=sonar" >> /opt/sonar/conf/sonar.properties
cat "sonar.jdbc.password=sonar" >> /opt/sonar/conf/sonar.properties
cat "sonar.jdbc.url=jdbc:mysql://{$ip}:3306/sonar?useUnicode=true&characterEncoding=utf8&rewriteBatchedStatements=true&useConfigs=maxPerformance" >>/opt/sonar/conf/sonar.properties
cat "sonar.web.host=0.0.0.0" >> /opt/sonar/conf/sonar.properties
cat "sonar.web.context=/sonar" >> /opt/sonar/conf/sonar.properties
cat "sonar.web.port=9000" >> /opt/sonar/conf/sonar.properties
cp /opt/sonar/bin/linux-x86-64/sonar.sh /etc/init.d/sonar
cat "SONAR_HOME=/opt/sonar" >> /etc/init.d/sonar
cat "PLATFORM=linux-x86-64" >> /etc/init.d/sonar
sed -i -e"s/^WRAPPER_CMD\s*=\s*./wrapper/WRAPPER_CMD = ${SONAR_HOME}/conf/wrapper.conf" /etc/init.d/sonar
